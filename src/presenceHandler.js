...
async function handlePresenceUpdate(oldPresence, newPresence) {
    try {
        ...
        if (oldStreaming && !newStreaming) {
            const removed = await removeLiveRole(member);
        ...
        if (!newStreaming) return;

        const streamTitle = (newStreaming.details || '').toLowerCase();

        console.log(`${member.user.tag} is streaming: ${newStreaming.details || 'Unknown Title'}`);
        ...
        const added = await addLiveRole(member);

        if (added) {
            await sendLog(
                member.guild,
                '🟢 Stream Detected',
                `Live Now role added.\n\n**Title:** ${newStreaming.details || 'Unknown Title'}`,
                member
            );
        }
